...
for sessionIndex = 1:5
    ...
    waveformSet = zeros( 1500, WAVSIZE + 5 ); % +1 speaker label, +2 emotion label, +3 gender label, +4 speech length, +5 global utterance index
    specSet = zeros( 1500, specHeigth *specWidth + 5 );
    dataIndex = 1;
    ...
    for fileIndex = 3: length( wavFile ) % 3 because the first two are dicts
        ...
        if isequal( wavFileName( 1:3 ), 'Ses' ) && isequal( wavFileName( end-3: end ), '.wav' )
           ...
           for utteranceIndex = 1: length( startTimeMatrix )
               ...
               audiowrite( [ '../../processedData/rawWav/', num2str(sampleRate),'/', num2str( sessionIndex ), '/', num2str( globalUtteranceIndex, '%06d' ), '.wav'], audioClip , Fs );
               ...
               waveformSet( dataIndex, : ) = [ audioClip, categoricalLabelMatrix( utteranceIndex ), speakerMatrix( utteranceIndex ), genderMatrix( utteranceIndex ), ( endTimeMatrix( utteranceIndex ) - startTimeMatrix( utteranceIndex ) ), globalUtteranceIndex ];
               specSet( dataIndex, : ) = [ audioSpec, categoricalLabelMatrix( utteranceIndex ), speakerMatrix( utteranceIndex ), genderMatrix( utteranceIndex ), ( endTimeMatrix( utteranceIndex ) - startTimeMatrix( utteranceIndex ) ), globalUtteranceIndex ];
               dataIndex = dataIndex + 1;
               globalUtteranceIndex = globalUtteranceIndex + 1;
           end
        end
    end
