...
def testNet( input, timeStep_num = 150, convLayer_num_front = 1, filter_num = 64, numClass = 4, init = 'glorot_uniform',\
            activationUnit = 'relu', conv_filter_size_front = 128, pooling_size = 2, convLayer_num_back = 4, conv_filter_size_back = 40, l2_reg = 0.01,\
            denseUnitNum = 64 ):
    ...
    input = tf.abs( input )
    input = keras.layers.pooling.AveragePooling2D( ( 1, pooling_size **(convLayer_num_front + 7 ) ), padding='valid' )( input )
    ...
    for i in range( convLayer_num_back ):
        input = tf.layers.batch_normalization( input )
        input = keras.layers.convolutional.Conv2D( 32, ( 3, 2 ), padding='same', activation= activationUnit, kernel_regularizer=regularizers.l2( l2_reg ), kernel_initializer = init )( input )
