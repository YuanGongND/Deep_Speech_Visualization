...
def train( testFeature, testLabel, trainFeature, trainLabel, newFolderName, iteration_num = 100, \
    ...
    with tf.Session() as sess:
        ...
        for iteration in range( 0, iteration_num ):
            ...
            plotTSNE( outputDense1, testSubsetLabel, newFolderName + '/figure/TSER/tSNE_2_' + str( iteration ) + '.png' )
      ...
      for layerIndex in range( len( layerList ) ):
          ...
          if layerIndex <= 7:
              ...
              if layerIndex == 0:
                  ...
                  plotRandomConvFilters( kernal, newFolderName + '/figure/TSNE' + layerList( layerIndex ) + '_' + str( iteration ) + '.png' )
...
def plotRandomConvFilters( inputM, saveFolder = '', plot_num = 16 ):
    ...
    fig, ax = plt.subplots( nrows= 1, ncols= output_channel_num )
    for randomIndex in selectedFilter:
            tempFilter = inputM[ :, :, randomIndex, randomIndex ]
            # if 1-D filter, waveform
            if np.shape( tempFilter )[ 0 ] == 1:
                tempFilter = tempFilter.reshape( np.shape( tempFilter )[ 1 ] )
