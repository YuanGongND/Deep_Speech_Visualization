...
#        print( tf.get_default_graph().get_name_scope() + str( input.shape ) )
        input = keras.layers.convolutional.Conv2D( filters = 16, kernel_size = ( 1, 64 ), strides= ( 1, 2 ), padding='same', activation= activationUnit, kernel_regularizer=regularizers.l2( l2_reg ), kernel_initializer = keras.initializers.Constant(value=0.005), bias_initializer = biasInit  )( input )
        print( tf.get_default_graph().get_name_scope() + str( input.shape ) )
