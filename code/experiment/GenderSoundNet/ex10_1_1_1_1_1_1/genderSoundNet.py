...
resultList = [  ]
#for learningRate in [ 1e-2, 5e-3, 1e-3, 5e-4, 1e-4, 5e-5, 1e-5 ]:
#    for batch_size in [ 16, 32, 64, 96 ]:
#        for init in [ 'RandomUniform', 'lecun_normal', 'lecun_uniform', 'glorot_normal', 'glorot_uniform', 'he_normal', 'he_uniform' ]:
for learningRate in [ 1e-2, 5e-3 ]:
    for batch_size in [ 16 ]:
        for init in [ 'RandomUniform' ]:
            tempFolderName = newFolderName + '/' + str( learningRate ) + '_' + str( batch_size ) + '_' + init
            os.mkdir( tempFolderName )
            # train the model
            resultOnTrain, resultOnTest = expUtil.train( testFeature, testLabel, trainFeature, trainLabel, iteration_num = 5, \
                                                        lr_decay = 0.1, batch_size = batch_size, learningRate = learningRate, iterationNum = 100, \
                                                        modelT = model, newFolderName = tempFolderName, init = init  )
            resultList.append( resultOnTest[ -1 ] )
            np.savetxt( 'gridSearch.csv', resultList, delimiter = ',' )
