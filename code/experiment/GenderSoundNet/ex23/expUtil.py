...
def train( testFeature, testLabel, trainFeature, trainLabel, newFolderName, iteration_num = 100, \
    ...
    with tf.Session() as sess:
        ...
        for iteration in range( 0, iteration_num ):
            ...
            if iteration == 19:
                plotALotTSNE( outputBeforeDense, testSubsetLabel, newFolderName + '/models/tSNE_3_' )
                plotALotTSNE( outputDense1, testSubsetLabel, newFolderName + '/models/tSNE_4_' )
...
def calculateTSNE( inputM, perplexity= 16.0, n_iter = 5000, learning_rate = 10 ):
    randomState = 7
    tsne = TSNE( random_state= randomState, perplexity= perplexity, n_iter = n_iter, learning_rate = learning_rate )
