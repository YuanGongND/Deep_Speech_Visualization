    ...
    with tf.Session() as sess:
        ...
        for iteration in range( 0, iteration_num ):
            ...
            for testBatch in range( 0, testBatchNum ): # 3*32=96 test samples
                ...
                tempTestResult, tempAccuracyTest, outputBeforeDense, outputDense1 = sess.run( [ prediction, accuracy, flattenOut, dense1Out ], feed_dict = { input_x: inputTestFeature, input_y: inputTestLabel } ) 
                ...
                plotTSNE( outputBeforeDense, np.argmax( inputTestLabel, 1 ), newFolderName + '/models/tSNE_1_' + str( iteration ) + '.png' )
                plotTSNE( outputDense1, np.argmax( inputTestLabel, 1 ), newFolderName + '/models/tSNE_2_' + str( iteration ) + '.png' )
...
def calculateTSNE( inputM ):
    randomState = 7
    tsne = TSNE( random_state= randomState, perplexity= 5.0, n_iter=5000 )
