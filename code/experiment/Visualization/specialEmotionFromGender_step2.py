...
def directAdv( modelName, inputTestFeature, inputTestLabel, eps, iteration_num, norm ):
    ...
    with tf.Session() as sess:
      ...
      for iteration in range( 1, iteration_num ):
          ...
          tempAdvTestFeature = iter_loadtxt( 'emotionSpecialGender/' + str(eps) + '_' + str(iteration) + 'adv.csv' )
...
#          tempNoiseTestFeature = iter_loadtxt( 'emotionSpecialGender/' + str(eps) + '_' + str(iteration) + 'noise.csv' )
...
for norm in [ 0 ]:
    ...
    for epsIndex in range( 0, len( epsList ) ):
        resultList1 = directAdv( folderName + '/model_90_.ckpt', testFeature, testLabel, epsList[ epsIndex ], iteration_num, norm )
...
#        plt.show()
        ...
        np.savetxt( 'emotionSpecialGender/' + str(norm) + 'RNNoverallAdvCont.csv', overallResultAdv, delimiter = ','  )
