...
for norm in [ 0 ]:
    ...
    #
    if norm == 0:
        epsList = np.multiply( list( range( 0, 20, 1 ) ), 0.001 )
    elif norm == 1:
...
#        plt.show()
        ...
        np.savetxt( newFolderName + '/' + str(norm) + 'overallAdv.csv', overallResultAdv, delimiter = ','  )
        np.savetxt( newFolderName + '/' + str(norm) + 'overallNoise.csv', overallResultNoise, delimiter = ','  )
