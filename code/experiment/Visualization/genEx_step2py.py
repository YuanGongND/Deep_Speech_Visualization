...
def directAdv( modelName, inputTestFeature, inputTestLabel, eps, iteration_num, norm ):
    ...
    with tf.Session() as sess:
      ...
      for iteration in range( 0, iteration_num ):
          ...
          tempAdvTestFeature = iter_loadtxt( 'genExEmotion/' + str(eps) + '_' + str(iteration) + 'adv.csv' )
          ...
          tempNoiseTestFeature = iter_loadtxt( 'genExEmotion/' + str(eps) + '_' + str(iteration) + 'noise.csv' )
...
for norm in [ 0 ]:
    ...
    #
    if norm == 0:
        epsList = np.multiply( list( range( 20, 40, 1 ) ), 0.001 )
    elif norm == 1:
    ...
    for epsIndex in range( 0, len( epsList ) ):
        resultList1, resultList2 = directAdv( folderName + '/model_90_.ckpt', testFeature, testLabel, epsList[ epsIndex ], iteration_num, norm )
...
#        plt.show()
        ...
        np.savetxt( 'genExp/' + str(norm) + 'RNNoverallAdvCont.csv', overallResultAdv, delimiter = ','  )
        np.savetxt( 'genExp/' + str(norm) + 'RNNoverallNoiseCont.csv', overallResultNoise, delimiter = ','  )
