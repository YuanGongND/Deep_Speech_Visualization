...
def loadData( testTask, testFolder = 4, precision = 'original', sampleRate = 16000 ):
    ...
    if testTask == 'emotion':
        trainFeature, trainEmotionLabel = processData( trainData, task = testTask, balance = 'balance' ) # emotion is not
    else:
        trainFeature, trainEmotionLabel = processData( trainData, task = testTask )
    testFeature, testEmotionLabel = processData( testData, task = testTask ) # note: don't balance the test set
