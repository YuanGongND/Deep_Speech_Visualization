...
def testNetRNN( input, timeStep_num = 150, convLayer_num_front = 1, filter_num = 64, numClass = 4, init = 'glorot_uniform',\
    ...
    print( input.shape )
    input = tf.transpose( input, [ 1, 2, 0 ] )
    print( input.shape )
    print('before')
    
    # start the LSTM layers 
    for i in range( 0 ):
        input = keras.layers.recurrent.LSTM( 64, activation= 'tanh', return_sequences=True)( input )
    input = keras.layers.recurrent.LSTM( 64, activation='tanh' )( input )
